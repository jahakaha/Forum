...
INNER JOIN user ON user.id = posts.user_id
WHERE (liked.liked = 1 and liked.disliked = 0) OR (liked.liked = 0 and liked.disliked = 1) OR posts.user_id = ?
	`, f.User.ID, f.User.ID)
	if err != nil {
		return err
	}
...
func (f *Forum) ReadComLikesDislikes() error {
	...
	WHERE (comlikes.liked = 1 AND comlikes.disliked = 0) OR (comlikes.liked = 0 AND comlikes.disliked = 1) OR comments.auth_id = ?
	`, f.User.ID, f.User.ID)
	if err != nil {
		return err
	}
...
INNER JOIN user ON user.id = posts.user_id
WHERE liked.liked = 1 AND liked.disliked = 0 AND liked.auth_id = ?
	`, f.User.ID)
	if err != nil {
		return err
	}
