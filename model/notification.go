...
//DeleteNotification sets new notifications to old
func (f *Forum) DeleteNotification() error {
	_, err := Db.Exec(`
...
func (f *Forum) ReadCountNotification() error {
	row := Db.QueryRow(`SELECT count(valid) FROM notific_liked 
	INNER JOIN posts ON notific_liked.post_id = posts.id 
	WHERE valid = 1 AND posts.user_id = ? AND notific_liked.from_auth != ?
	`, f.User.ID, f.User.ID)
	...
	row2 := Db.QueryRow(`SELECT count(valid) FROM notific_comment 
	INNER JOIN posts ON notific_comment.post_id = posts.id 
	WHERE valid = 1 AND posts.user_id = ? AND notific_comment.from_auth != ?
	`, f.User.ID, f.User.ID)
