...
func (c *Comment) DeleteCommentFromDataBase(userid int) error {
	...
	DELETE FROM comlikes WHERE comlikes.com_id = $1 AND 
(SELECT comments.auth_id FROM comments 
 INNER JOIN comlikes ON comlikes.com_id = comments.id WHERE comlikes.com_id = $2) = $3;
 DELETE FROM notific_comment WHERE notific_comment.com_id = $4 and notific_comment.to_auth = $5;	
 DELETE FROM comments WHERE comments.id = $6 and comments.auth_id = $7;
	`, c.ID, c.ID, c.AuthID, c.ID, c.AuthID, c.ID, c.AuthID)
	if err != nil {
		tx.Rollback()
		return err
	}
